...
public class StreamsExample {
    ...
    public static void main(final String[] args) {
        ...
        Consumer<Author> authorPrintConsumer1 = new Consumer<Author>() {
            @Override
            public void accept(Author author) {
                System.out.println(author);
            }
        };
        authors.stream().filter(activeAuthors).forEach(authorPrintConsumer1);
        ...
        Consumer<Book> authorConsumer = new Consumer<Book>() {
            @Override
        ...
        Function<Author, Stream<Book>> priceForAllBooks = new Function<Author, Stream<Book>>() {
            @Override
            public Stream<Book> apply(Author author) {
                return author.books.stream();
            }
        };
        ToDoubleFunction<Book> mapToDouble = new ToDoubleFunction<Book>() {
            @Override
        ...
        OptionalDouble averagePrice = authors.stream()
                        .flatMap(priceForAllBooks)
                                .mapToDouble(mapToDouble)
                                        .average();
        System.out.println("Average is: "+ averagePrice.getAsDouble());


        banner("Average price for all books in the library - lambda");
        // TODO With functional interfaces used directly
        OptionalDouble averagePrice1 = authors.stream()
                        .flatMap(author -> author.books.stream())
                                .mapToDouble(book -> book.price)
                                        .average();
        System.out.println("Average is: "+ averagePrice1.getAsDouble());



        banner("Active authors that have at least one published book");
        // TODO With functional interfaces declared
        Predicate<Book> publishedBooks2 = new Predicate<Book>() {
            @Override
            public boolean test(Book book) {
                return book.published;
            }
        };
        Predicate<Author> activeAuthorsHaveAtLeastOnePublishedBook = new Predicate<Author>() {

            @Override
            public boolean test(Author author) {
                return author.active && author.books.stream().anyMatch(publishedBooks);
            }
        };
        Consumer<Author> printAuthors = new Consumer<Author>() {
            @Override
            public void accept(Author author) {
                System.out.println(author);
            }
        ...
        authors.stream()
                        .filter(activeAuthorsHaveAtLeastOnePublishedBook)
                                .forEach(printAuthors);

        banner("Active authors that have at least one published book - lambda");
        // TODO With functional interfaces used directly
        authors
                .stream()
                .filter(author -> author.active && author.books.stream().anyMatch(
                        book -> book.published
                )).forEach(System.out::println);

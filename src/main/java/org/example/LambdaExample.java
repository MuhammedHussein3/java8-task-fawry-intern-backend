...
public class LambdaExample {
    ...
    public static void main(final String[] args) {
        ...
        Predicate<User> usersAgeGreaterThanFive = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.age > 5;
            }
        };
        Comparator<User> sortByUserName = new Comparator<User>() {
            @Override
            public int compare(User o1, User o2) {
                return o1.name.compareTo(o2.name);
            }
        };
        repository.select(usersAgeGreaterThanFive, sortByUserName);
        ...
        // TODO With functional interfaces declared
        Predicate<User> usersAgeLessThanTen = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.age < 10;
            }
        ...
        repository.select(usersAgeLessThanTen,userComparator);
        ...
        // TODO With functional interfaces declared
        Predicate<User> usersActive = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.active;
            }
        };
        Comparator<User> sortedUsersByName = new Comparator<User>() {
            @Override
            public int compare(User o1, User o2) {
                return o1.name.compareTo(o2.name);
            }
        };
        repository.select(usersActive, sortedUsersByName);
        ...
        // TODO With functional interfaces declared
        Predicate<User> usersAgeGreaterThanEight = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.age > 8;
            }
        };
        Comparator<User> usersSortedByNameWithAgeGreaterThanEight = new Comparator<User>() {
            @Override
            public int compare(User o1, User o2) {
                return o1.name.compareTo(o2.name);
            }
        };
        repository.select(usersAgeGreaterThanEight, usersSortedByNameWithAgeGreaterThanEight);
